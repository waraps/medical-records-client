...
import { Button, Flex, Heading, Spinner, useToast, Text, Box, Avatar, FormControl, FormLabel, Input, FormErrorMessage, Select } from '@chakra-ui/react';
...
export const DetailsUser = (): JSX.Element => {
    const { id } = useParams();
    const { data: roles } = useFetch<IRol[]>('/roles');
    const { fetchData, loading, error, data: user} = useFetch<IUser>(`/users/${id}`, undefined, false);
    const { doUpdate, loading: loadingUpdated, error: errorUpdated, data: userUpdated } = usePut<IUser, IUserReq>(`/users/${id}`);
    const toast = useToast();

    useEffect(() => fetchData(), []);

    const { register, handleSubmit, formState: { errors, dirtyFields }, reset} = useForm<editUserSchemaType>({
        mode: 'onSubmit',
